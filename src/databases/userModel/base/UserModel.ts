...
export interface UserModel {
	...
	readonly first_seen_at_steam: Date | null;

	readonly last_seen_at_steam: Date | null;
...
export function convertToUser(model: UserModel): User {
	...
	if (model.first_seen_at_steam !== null && model.last_seen_at_steam !== null) {
		steamMeta = {
			firstSeenAt: model.first_seen_at_steam.toISOString(),
			lastSeenAt: model.last_seen_at_steam.toISOString(),
		};
	}
